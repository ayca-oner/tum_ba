...
uint8_t send_response_packet_measurement(uint8_t *data, uint8_t length);
...
//Added by Nico
uint8_t  indexMeasurement = 0, meas[48] = {0}; //Needed for measurement
//uint8_t times[50], offsets[50], channels[50], ;
...
enum control_commands
{
    ...
    DUMP_RADIO_PACKETS,
    RESET_BOARD,
	GET_MEASUREMENT,   //Added by Nico to measure ieee154e access time and channel
	RESET_MEASUREMENT, //Added by Nico to reset measurement variables
	TOGGLE_MEASUREMENT //Added by Nico to toggle measurement mode
};
...
void openserial_init() {
	...
	//Initiate measurement variables 
	/*uint8_t i;
	for(i=0; i < 50; i++)
	{
		channels[i] = 16;
...
uint8_t handle_control_commands() {
	uint8_t nbr_count, index, nbr_list[40], slots[8] = { 0,0,0,0,0,0,0,0 }, vars[5] ={0,0,0,0,0}, node_type = FALSE, slotframeID, control_cmd_byte;
            ...
            for(i = 0;i<maxActiveSlots;i++)
            {
                ...
                else
                {
                    if(sE->type == CELLTYPE_TX)
                        ...
                        slots[4]++;
                    else if(sE->type == CELLTYPE_MAC_RESOLUTION)
                        slots[5]++;
                    else if(sE->type == CELLTYPE_MAC_INIT)
                        slots[6]++;
                    else if(sE->type == CELLTYPE_SERIALTX)
                        slots[7]++;

                }
            }
            send_response_packet(slots,7);

			//Added by Nico to get a more detailed answer on the schedule
			get_advanced_schedule_data();
...
//===================Added by Nico=======================================
		case GET_MEASUREMENT:
			/*send_response_packet_measurement(channels, 50);
			send_response_packet_measurement(times, 50);
			send_response_packet_measurement(offsets, 50);
			vars[0] = ieee154e_getTimesSent();
			vars[1] = ieee154e_getResSlotsTaken();
			send_response_packet(vars, 2);
			break;*/
		        /*asn = ieee154e_getASN();
			vars[0] = asn.byte4;
			vars[1] = asn.bytes2and3 >> 8;
			vars[2] = asn.bytes2and3;
			vars[3] = asn.bytes0and1 >> 8;
			vars[4] = asn.bytes0and1;
			send_response_packet(vars,5);
			addr = idmanager_getMyID(ADDR_64B);
			vars[0] = addr->prefix[0];
			vars[1] = addr->prefix[1];
			vars[2] = addr->prefix[2];
			vars[3] = addr->prefix[3];
			vars[4] = addr->prefix[4];
			send_response_packet(vars,5);*/

			send_response_packet(meas,18);			 
		...
		case TOGGLE_MEASUREMENT:
			vars[0] = ieee154e_toggleMeasurement(); //1 if disabled, 2 when enabled
			send_response_packet(vars,2);
			break;
...
void openserial_sendMeasurement()
{
	if(meas[0] != 0)
	{
...
/** 
// Added by Nico to set measurement variables
*/
void openserial_setMeasurementVariables(uint8_t channel, asn_t asntx, asn_t asnactual, uint8_t offset, char *eui64b, uint8_t seq)
{
	...
	/*data[12] = asntx.byte4;
	data[13] = asntx.bytes2and3 >> 8;
	data[14] = asntx.bytes2and3;
	data[15] = asntx.bytes0and1 >> 8;
	data[16] = asntx.bytes0and1;

	data[12] = asnactual.byte4;
	data[13] = asnactual.bytes2and3 >> 8;
	data[14] = asnactual.bytes2and3;
	data[15] = asnactual.bytes0and1 >> 8;
	data[16] = asnactual.bytes0and1;
    ...
    // ayca burdan comment basliyor
		...
		for (i = 4; i < 12; i++)
		{
			meas[indexMeasurement+i] = eui64b[i +4];
		}

		meas[12] = asnactual.byte4;
		meas[13] = asnactual.bytes2and3 >> 8;
		meas[14] = asnactual.bytes2and3;
		meas[15] = asnactual.bytes0and1 >> 8;
...
/**
//Added by Nico to send longer measurement response
*/
uint8_t send_response_packet_measurement(uint8_t *data, uint8_t length)
{
	uint8_t resp[60] = { 0 };
...
/**
//Added by Nico to get more advanced schedule information
*/
void get_advanced_schedule_data()
{
	uint8_t slotInfo[8] = { 0,0,0,0,0,0,0,0 }, slotframeID, freqInfo[2] = { 0,0 };
	uint16_t maxActiveSlots, i = 0;
	scheduleEntry_t* sE;
	...
	for (i = 0; i < maxActiveSlots; i++)
	{
		...
		{
			if (sE->type == CELLTYPE_OFF)
			{
				slotInfo[0] = 0;
			}
			else if(sE->type == CELLTYPE_TX)
			{
				slotInfo[0] = 1;
			}
			else if (sE->type == CELLTYPE_RX)
			...
			else if (sE->type == CELLTYPE_MORESERIALRX)
			{
				slotInfo[0] = 5;
			}
			else if (sE->type == CELLTYPE_MAC_RESOLUTION)
			...
			slotInfo[1] = sE->channelOffset;
			slotInfo[2] = sE->numRx;
			slotInfo[3] = sE->numTx;
			slotInfo[4] = sE->numTxACK;
			slotInfo[5] = sE->slotOffset;
			send_response_packet(slotInfo, 8);
		}
	}
	
	uint8_t hallo = ieee154e_getFreqAna();
	uint8_t hallo1 = ieee154e_getSentInMacInit();
	freqInfo[0] = hallo;
        freqInfo[1] = hallo1;
	send_response_packet(freqInfo, 2);
...
uint8_t inject_udp_packet()
{
	...
	//Outcommented by Nico to allow Anycast from DAGroot

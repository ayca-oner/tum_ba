...
void schedule_init() {
   ...
   start_slotOffset = SCHEDULE_MINIMAL_6TISCH_SLOTOFFSET+1;
...
/*
\brief Adding a sync slot on normal frequency with slotoffset 2,
\as = is beacon and 1 serial Rx.
\added by Nico
*/
void schedule_addSyncSlot()
{
	open_addr_t     temp_neighbor;
	memset(&temp_neighbor, 0, sizeof(temp_neighbor));
	temp_neighbor.type = ADDR_ANYCAST;
...
/*
\brief Adding two resolution slots for MAC initiation, when 
\starting DAGroot, for resolving resolution.
\Added by Nico
*/
void schedule_addResolutionSlots()
{
	uint8_t i;

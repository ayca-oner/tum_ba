...
//Added by Nico to measure the things needed
//Note: Most of the variables are not needed in the current state but will 
//be left here for future. ResEnabled is used for enable measurement
typedef struct 
{
	bool					 resEnabled;			  //whether the measurement for resolution slots gambling is enabled 
	uint32_t					 startTime;				  //Starting time measurement
	uint32_t					 endTime;				  //Ending time measurement
	uint8_t                  index;					  //Index used at the time
	uint8_t					 resSlotTaken;            //How often the Resolution slot was taken
	uint8_t					 timesSent;				  //How often something has been send (only in MAC_INIT and RESOLUTION)
	uint8_t					 timersCount;			  //How often went the timer through?
	opentimers_id_t			 timer;					  //Timer used for measuring
	bool					 finished;				  //Checks whether measurement has finished

	uint8_t					 times;				  //Saves the times needed to find a resolution slot
	uint8_t					 channel;			  //Saves the used channel to transmit on
	uint8_t					 offset;			  //Saves the used Res Slot offset
} ieee154e_measurement_t;

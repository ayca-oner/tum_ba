...
//Added by Nico
uint8_t sentInMacInit = 0; //Counts how often something has been sent in this slot-used for debugging purposes
uint8_t needResSlot = FALSE; //Indicates if a resolution slot is needed
uint8_t resSlot = 0; //Determines the used resolution slot
ieee154e_measurement_t ieee154e_measurement; //Measurement variables
...
void ieee154e_init() {
	...
	ieee154e_measurement.timer = opentimers_create();
	ieee154e_resetMeasurement();
}
...
/**
//Added by Nico to get the channel to transmit on
*/
uint8_t ieee154e_getFreqAna()
{
	return ieee154e_vars.singleChannel;
}

/**
//Added by Nico to get the number of sent bytes
*/
uint8_t ieee154e_getSentInMacInit()
{
	return sentInMacInit;
}

/**
//Added by Nico to get the times sent in INIT or RESOLUTION slot
*/
uint8_t ieee154e_getTimesSent()
{
	return ieee154e_measurement.timesSent;
}

/**
//Added by Nico to get the times sent in RESOLUTION slot
*/
uint8_t ieee154e_getResSlotsTaken()
{
	return ieee154e_measurement.resSlotTaken;
}

/**
//Added by Nico to get the current asn
*/
asn_t ieee154e_getASN()
{
	return ieee154e_vars.asn;
}

/**
//Added by Nico to get the measured delays
*/
uint8_t* ieee154e_getMeasuredTimes()
{
	return ieee154e_measurement.times;
}

/**
//Added by Nico to get the channels transmitted on
*/
uint8_t* ieee154e_getMeasuredChannels()
{
	return ieee154e_measurement.channel;
}

/**
//Added by Nico to reset measurement variables
*/
void ieee154e_resetMeasurement()
{
	ieee154e_measurement.resEnabled = FALSE;
	ieee154e_measurement.startTime = 0;
	ieee154e_measurement.endTime = 0;
	ieee154e_measurement.index = 0;
	ieee154e_measurement.channel = 0;
	ieee154e_measurement.times = 0;
	ieee154e_measurement.resSlotTaken = 0;
	ieee154e_measurement.offset = 0;
}

void isr_ieee154e_handle_measurement_timer()
{

}

/**
//Added by Nico to get measurement variables
*/
ieee154e_measurement_t ieee154e_getMeasurement()
{
	return ieee154e_measurement;
}


/**
//Added by Nico to set measurement enabled/disabled
*/
uint8_t ieee154e_toggleMeasurement()
{
	if (ieee154e_measurement.resEnabled == FALSE)
	{
		ieee154e_measurement.resEnabled = TRUE;
	}
	else
	{
		ieee154e_measurement.resEnabled = FALSE;
...
port_INLINE void activity_ti1ORri1() {
	...
	//Added by Nico to prevent overflow
	if(ieee154e_measurement.resSlotTaken == 250 || ieee154e_measurement.timesSent == 250)
	{
		ieee154e_measurement.resSlotTaken = 0;
		ieee154e_measurement.resSlotTaken = 0;
	}
	...
	switch (cellType) {
		//====================================================================Added by Nico====================================================================
		//The first case is the Init field, where every mote can send something on.
	case CELLTYPE_MAC_INIT:

		//No Res slot needed yet
		needResSlot = FALSE;
	...
	case CELLTYPE_MAC_RESOLUTION:
		if (needResSlot == TRUE)
		{

			//See if it has been gambled
			if (resSlot == 0)
			{
				ieee154e_measurement.startTime = sctimer_readCounter();
				//Gamble if i take this slot or not
				uint16_t randomvalue = openrandom_get16b();
				randomvalue = randomvalue & 0x0001;

				if (randomvalue == 0) //Send in my slot
				{

					ieee154e_measurement.offset = 1;
					resSlot = 1;
				}
				else
				{
					ieee154e_measurement.offset = 2;
					resSlot = 1; //send next slot
					endSlot();
					return;
				}
			...
			if (resSlot == 1)
			{
				//Here we send in Res Slot
				ieee154e_measurement.resSlotTaken++;
				ieee154e_measurement.timesSent++;
	...
	case CELLTYPE_TX:
		...
		if (idmanager_getIsDAGroot() == FALSE) { //I am not DAGroot, i am not supposed to send here hence i will just listen
			// stop using serial
			openserial_stop();
...
port_INLINE void activity_ti2() {
	...
	// add 2 CRC bytes only to the local copy as we end up here for each retransmissionif
	packetfunctions_reserveFooterSize(&ieee154e_vars.localCopyForTransmission, 2);
#endif
	...
	//Added by Nico-necessary for measurement
	if ((schedule_getType() == CELLTYPE_MAC_INIT || schedule_getType() == CELLTYPE_MAC_RESOLUTION) && ieee154e_measurement.resEnabled == TRUE)
	{
		ieee154e_measurement.channel = ieee154e_vars.freq;
	}
...
port_INLINE void activity_ti3() {
	...
	//Added by Nico to get the ASN of the sent package
	if(idmanager_getIsDAGroot() == FALSE)
	{
		//Added by Nico to see, in which slot the packet has been sent
		uint8_t data[15];
		data[0] = 0x43;
		data[1]	= 0x44;
		data[2] = ieee154e_vars.localCopyForTransmission.l2_dsn;
		data[3] = ieee154e_vars.localCopyForTransmission.l2_asn.byte4;
		data[4] = ieee154e_vars.localCopyForTransmission.l2_asn.bytes2and3 >> 8;
		data[5] = ieee154e_vars.localCopyForTransmission.l2_asn.bytes2and3;
		data[6] = ieee154e_vars.localCopyForTransmission.l2_asn.bytes0and1 >> 8;
		data[7] = ieee154e_vars.localCopyForTransmission.l2_asn.bytes0and1;
		data[8] = 0xff;
		data[9] = ieee154e_vars.asn.byte4;
		data[10] = ieee154e_vars.asn.bytes2and3 >> 8;
		data[11] = ieee154e_vars.asn.bytes2and3;
		data[12] = ieee154e_vars.asn.bytes0and1 >> 8;
		data[13] = ieee154e_vars.asn.bytes0and1;
		data[14] = ieee154e_vars.freq;
		
		openserial_printf(data,15, 'R');
	}
...
port_INLINE void activity_ti5(PORT_TIMER_WIDTH capturedTime) {
	...
	//Added by Nico-necessary for slot resolution
	if (schedule_getType() == CELLTYPE_MAC_INIT || schedule_getType() == CELLTYPE_MAC_RESOLUTION)
	{
		listenForAck = TRUE;
	}
...
port_INLINE void activity_ti9(PORT_TIMER_WIDTH capturedTime) {
	...
	*/
	do { // this "loop" is only executed once
		...
		//Added by Nico		
		//Add to measurementlist if successfully sent in MAC_RESOLUTION slot 
		/*if (schedule_getType() == CELLTYPE_MAC_RESOLUTION && ieee154e_measurement.resEnabled == TRUE)
		{
			if (sentInMacInit > 50)
			{
				sentInMacInit = 0;
			...
			openserial_setMeasurementVariables(ieee154e_measurement.channel, ieee154e_measurement.times);
			ieee154e_measurement.endTime = sctimer_readCounter();
			ieee154e_measurement.times = (uint8_t)(ieee154e_measurement.endTime - ieee154e_measurement.startTime);
		}*/
...
port_INLINE void activity_ri5(PORT_TIMER_WIDTH capturedTime) {
	...
	*/
	do { // this "loop" is only executed once
		...
		//     openserial_printf(ieee154e_vars.dataReceived->payload,ieee154e_vars.dataReceived->length,'P');
		...
		if (ieee802514_header.ackRequested == 1 && ieee154e_vars.isAckEnabled == TRUE) {
			...
			radiotimer_setCapture(ACTION_TX_SEND_DONE);
#else
			// arm rt5 (Here although rt5 starts from here most of its times used in processing the packet)
			opentimers_scheduleAbsolute(
				ieee154e_vars.timerId,                            // timerId
